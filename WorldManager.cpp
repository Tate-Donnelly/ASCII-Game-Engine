    ...
    int WorldManager::moveObject(Object* p_o, Vector where) {
        ...
        if (u.boxIntersectsBox(orig_box, getBoundary()) && // Was in bounds?
            !(u.boxIntersectsBox(new_box, getBoundary()))) {  // Now out of bounds?
                ...
                p_o->eventHandler(&ov);  
                //
                LM.writeLog(0,"Orig_box %f %f", orig_box.getCorner().getX(), orig_box.getCorner().getY());
                LM.writeLog(0,"New_box %f %f", new_box.getCorner().getX(), new_box.getCorner().getY());
                LM.writeLog(0,"Getboundary %f %f Width %f Height %f", getBoundary().getCorner().getX(), getBoundary().getCorner().getY(), getBoundary().getHorizontal(), getBoundary().getVertical());                // Send to Object
        }
